...
[ExecuteAlways]
#endif
public class CirclesConnectionsWithTextShowing : CirclesConnection
{
    [Header("Intro Texts")]
    ...
    protected override void Update()
    {
        ...
        if (renderingCamera != null)
        {
            for (int i = 0; i < localPoints.Count; i++)
            {
                ...
                input.z = (renderingCamera.transform.position - transform.position).magnitude;
                ...
                //Ray *** Not Wrking but it works with UIScroll04
                //Vector3 pos = Input.mousePosition;
                //pos.z = renderingCamera.nearClipPlane;
                //Ray ray = renderingCamera.ScreenPointToRay(pos);
                //Vector3 hitPoint = ray.direction * (renderingCamera.transform.position - transform.position).magnitude;

                //GameObject sphere = GameObject.CreatePrimitive(PrimitiveType.Sphere);
                //sphere.transform.position = hitPoint;
                //Vector3 localPoint = transform.TransformPoint(localPoints[i]);
                //if (hitPoint.x < localPoint.x + hoverRadius &&
                //    hitPoint.x > localPoint.x - hoverRadius &&
                //    hitPoint.y < localPoint.y + hoverRadius &&
                //    hitPoint.y > localPoint.y - hoverRadius)
                //{
                //    isShown[i] = true;
                //}
